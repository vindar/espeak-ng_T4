...
FLASHMEM const char *ucd_get_category_group_string(ucd_category_group c)
{
	switch (c)
	{
...
FLASHMEM const char *ucd_get_category_string(ucd_category c)
{
	switch (c)
	{
...
PROGMEM static const char* uuscripts[] =
	{
		"Adlm",
		"Afak",
		"Aghb",
...
FLASHMEM const char *ucd_get_script_string(ucd_script s)
{

	if ((unsigned int)s >= (sizeof(uuscripts)/sizeof(uuscripts[0])))
		return "----";
	return uuscripts[(unsigned int)s];
}
