...
FLASHMEM int ucd_isalnum(codepoint_t c)
{
...
FLASHMEM int ucd_isalpha(codepoint_t c)
{
	ucd_category cat = ucd_lookup_category(c);
	switch (cat)
	{
...
FLASHMEM int ucd_isblank(codepoint_t c)
{
...
FLASHMEM int ucd_iscntrl(codepoint_t c)
{
	return ucd_lookup_category(c) == UCD_CATEGORY_Cc;
}

FLASHMEM int ucd_isdigit(codepoint_t c)
{
	return (c >= 0x30 && c <= 0x39); /* [0-9] */
}

FLASHMEM int ucd_isgraph(codepoint_t c)
{
	switch (ucd_lookup_category(c))
	{
	case UCD_CATEGORY_Cc:
...
FLASHMEM int ucd_islower(codepoint_t c)
{
	ucd_category cat = ucd_lookup_category(c);
	switch (cat)
	{
...
FLASHMEM int ucd_isprint(codepoint_t c)
{
	switch (ucd_lookup_category(c))
	{
	case UCD_CATEGORY_Cc:
...
FLASHMEM int ucd_ispunct(codepoint_t c)
{
	return ucd_isgraph(c) && !ucd_isalnum(c);
}

FLASHMEM int ucd_isspace(codepoint_t c)
{
	switch (ucd_lookup_category(c))
	{
	case UCD_CATEGORY_Zl:
...
FLASHMEM int ucd_isupper(codepoint_t c)
{
	ucd_category cat = ucd_lookup_category(c);
	switch (cat)
	{
...
FLASHMEM int ucd_isxdigit(codepoint_t c)
{
	return (c >= 0x30 && c <= 0x39)  /* [0-9] */
	    || (c >= 0x41 && c <= 0x46)  /* [A-Z] */
	    || (c >= 0x61 && c <= 0x66); /* [a-z] */
