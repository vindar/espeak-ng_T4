...
PROGMEM static const keywtab_t k_conditions[] = {
	{ "AND",  0, k_AND },
	{ "OR",   0, k_OR },
	{ "THEN", 0, k_THEN },
	{ "NOT",  0, k_NOT },
...
PROGMEM static const keywtab_t k_properties[] = {
...
PROGMEM static const unsigned char utf8_bom[] = { 0xef, 0xbb, 0xbf, 0 };

PROGMEM static const keywtab_t k_intonation[] = {
	{ "tune",       0, kTUNE },
	{ "endtune",    0, kENDTUNE },
...
PROGMEM static const keywtab_t keywords[] = {
...
FLASHMEM static void clean_context(CompileContext *ctx) {
	...
	for (int i = 0; i < ctx->n_manifest; i++) {
		my_free(ctx->manifest[i].name);
	}
	my_free(ctx->manifest);
	my_free(ctx);
}

FLASHMEM static void error(CompileContext *ctx, const char *format, ...)
{
	va_list args;
	va_start(args, format);

	my_fprintf(ctx->f_errors, "%s(%d): ", ctx->current_fname, ctx->linenum-1);
	my_vfprintf(ctx->f_errors, format, args);
	my_fprintf(ctx->f_errors, "\n");
	ctx->error_count++;

	va_end(args);
}

FLASHMEM static void error_from_status(CompileContext *ctx, espeak_ng_STATUS status, const char *context)
{
	char message[512];
...
FLASHMEM static espeak_ng_STATUS ReadPhondataManifest(CompileContext *ctx, espeak_ng_ERROR_CONTEXT *context)
{
	...
	while (my_fgets(buf, sizeof(buf), f) != NULL)
		n_lines++;
	...
	NAMETAB *new_manifest = (NAMETAB *)my_realloc(ctx->manifest, n_lines * sizeof(NAMETAB));
	...
	while (my_fgets(buf, sizeof(buf), f) != NULL) {
		...
		if (sscanf(&buf[2], "%x %s", &value, name) == 2) {
			if ((p = (char *)my_malloc(strlen(name)+1)) != NULL) {
				strcpy(p, name);
...
PROGMEM static const MNEM_TAB reserved_phonemes[] = {
...
FLASHMEM static void ReservePhCodes(CompileContext *ctx)
{
...
FLASHMEM static int LookupPhoneme(CompileContext *ctx, const char *string, int control)
{
...
FLASHMEM static unsigned int get_char(CompileContext *ctx)
{
	unsigned int c;
	c = my_fgetc(ctx->f_in);
	if (c == '\n')
...
FLASHMEM static void unget_char(CompileContext *ctx, unsigned int c)
{
	my_ungetc(c, ctx->f_in);
	if (c == '\n')
		ctx->linenum--;
}

FLASHMEM static int CheckNextChar(CompileContext *ctx)
{
	int c;
...
FLASHMEM static int NextItem(CompileContext *ctx, int type)
{
	int acc;
	unsigned char c = 0;
	unsigned char c2;
	...
	ctx->f_in_displ = my_ftell(ctx->f_in);
	ctx->f_in_linenum = ctx->linenum;

	while (!my_feof(ctx->f_in)) {
		c = get_char(ctx);
		if (c == '/') {
			if ((c2 = get_char(ctx)) == '/') {
				// comment, ignore to end of line
				while (!my_feof(ctx->f_in) && ((c = get_char(ctx)) != '\n'))
					;
			} else
				unget_char(ctx, c2);
		}
	...
	ix = 0;
	while (!my_feof(ctx->f_in) && !isspace(c) && (c != '(') && (c != ')') && (c != ',')) {
		if (c == '\\')
			c = get_char(ctx);
...
FLASHMEM static int NextItemMax(CompileContext *ctx, int max)
{
...
FLASHMEM static int NextItemBrackets(CompileContext *ctx, int type, int control)
{
...
FLASHMEM static void UngetItem(CompileContext *ctx)
{
	my_fseek(ctx->f_in, ctx->f_in_displ, SEEK_SET);
	ctx->linenum = ctx->f_in_linenum;
}

FLASHMEM static int Range(int value, int divide, int min, int max)
{
	if (value < 0)
		value -= divide/2;
	else
...
FLASHMEM static int CompileVowelTransition(CompileContext *ctx, int which)
{
...
FLASHMEM static espeak_ng_STATUS LoadSpect(CompileContext *ctx, const char *path, int control, int *addr)
{
	SpectSeq *spectseq;
	int peak;
	int frame;
	...
	*addr = my_ftell(ctx->f_phdata);
	...
	if (klatt_flag) {
		...
		my_fwrite(&seqk_out, ix, 1, ctx->f_phdata);
		while (ix & 3)
		{
			// round up to multiple of 4 bytes
			my_fputc(0, ctx->f_phdata);
			ix++;
		}
	} else {
		ix = (char *)(&seq_out.frame[seq_out.n_frames]) - (char *)(&seq_out);
		my_fwrite(&seq_out, ix, 1, ctx->f_phdata);
		while (ix & 3)
		{
			// round up to multiple of 4 bytes
			my_fputc(0, ctx->f_phdata);
...
FLASHMEM static int LoadWavefile(CompileContext *ctx, MY_FILE *f, const char *fname)
{
	int displ;
	unsigned char c1;
	unsigned char c3;
	...
	my_fseek(f, 24, SEEK_SET);
	sr1 = Read4Bytes(f);
	sr2 = Read4Bytes(f);
	my_fseek(f, 40, SEEK_SET);
	...
	while (!my_feof(f)) {
		...
		if (scale_factor <= MIN_FACTOR) {
			my_fputc(sample & 0xff, ctx->f_phdata);
			my_fputc(sample >> 8, ctx->f_phdata);
		} else {
			...
			if (sample2 < -128)
				sample2 = -128;
			my_fputc(sample2, ctx->f_phdata);
		}
	}

	length = my_ftell(ctx->f_phdata);
	while ((length & 3) != 0) {
...
FLASHMEM static espeak_ng_STATUS LoadEnvelope(CompileContext *ctx, MY_FILE *f, int *displ)
{
	char buf[128];
	...
	if (my_fseek(f, 12, SEEK_SET) == -1)
		return errno;

	if (my_fread(buf, 128, 1, f) != 128)
		return errno;
	my_fwrite(buf, 128, 1, ctx->f_phdata);

	return ENS_OK;
}

// Generate a hash code from the specified string
FLASHMEM static int Hash8(const char *string)
{
	int c;
	int chars = 0;
	int hash = 0;
...
FLASHMEM static int LoadEnvelope2(CompileContext *ctx, MY_FILE *f)
{
	int ix, ix2;
	int n;
	int x, y;
	...
	n_points = 0;
	if (my_fgets(line_buf, sizeof(line_buf), f) != NULL) { ; // skip first line, then loop
		while (!my_feof(f)) {
			if (my_fgets(line_buf, sizeof(line_buf), f) == NULL)
				break;
	...
	displ = my_ftell(ctx->f_phdata);
	my_fwrite(env, 1, ENV_LEN, ctx->f_phdata);

	return displ;
}

FLASHMEM static espeak_ng_STATUS LoadDataFile(CompileContext *ctx, const char *path, int control, int *addr)
{
	...
	if (*addr == 0) {
		...
		if (*addr > 0)
			my_fprintf(ctx->f_phcontents, "%c  0x%.5x  %s\n", type_code, *addr & 0x7fffff, path);
	}

	// add this item to the hash table
	if (*addr > 0) {
		p = ctx->ref_hash_tab[hash];
		p2 = (REF_HASH_TAB *)my_malloc(sizeof(REF_HASH_TAB)+strlen(path)+1);
		if (p2 == NULL)
			return ENOMEM;
		p2->value = *addr;
...
FLASHMEM static void CompileToneSpec(CompileContext *ctx)
{
	int pitch1 = 0;
	int pitch2 = 0;
	int pitch_env = 0;
...
FLASHMEM static void CompileSound(CompileContext *ctx, int keyword, int isvowel)
{
	int addr = 0;
	int value = 0;
	char path[N_ITEM_STRING];
	PROGMEM static const int sound_instns[] = { i_FMT, i_WAV, i_VWLSTART, i_VWLENDING, i_WAVADD };
...
/*
   ...
   =9         special tests
 */
FLASHMEM static int CompileIf(CompileContext *ctx, int elif)
{
	int key;
	bool finish = false;
	int word = 0;
...
FLASHMEM static void FillThen(CompileContext *ctx, int add)
{
	unsigned short *p;
	int offset;
...
FLASHMEM static int CompileElse(CompileContext *ctx)
{
	unsigned short *ref;
	unsigned short *p;
...
FLASHMEM static int CompileElif(CompileContext *ctx)
{
	if (ctx->if_level < 1) {
...
FLASHMEM static int CompileEndif(CompileContext *ctx)
{
	unsigned short *p;
	int chain;
	int offset;
...
FLASHMEM static int CompileSwitch(CompileContext *ctx, int type)
{
	// Type 0:  EndSwitch
...
FLASHMEM static PHONEME_TAB_LIST *FindPhonemeTable(CompileContext *ctx, const char *string)
{
	int ix;
...
FLASHMEM static PHONEME_TAB *FindPhoneme(CompileContext *ctx, const char *string)
{
...
FLASHMEM static void ImportPhoneme(CompileContext *ctx)
{
...
FLASHMEM static void CallPhoneme(CompileContext *ctx)
{
	PHONEME_TAB *ph;
	int ix;
	int addr = 0;
...
FLASHMEM static void DecThenCount(CompileContext *ctx)
{
	if (ctx->then_count > 0)
		ctx->then_count--;
}

FLASHMEM static int CompilePhoneme(CompileContext *ctx, int compile_phoneme)
{
	int endphoneme = 0;
	int keyword;
	int value;
	...
	while (!endphoneme && !my_feof(ctx->f_in)) {
	...
	if (ctx->prog_out > ctx->prog_buf) {
		...
		my_fflush(ctx->f_phindex);
		ctx->phoneme_out->program = my_ftell(ctx->f_phindex) / sizeof(unsigned short);

		if (ctx->f_prog_log != NULL) {
			phoneme_prog_log.addr = ctx->phoneme_out->program;
			phoneme_prog_log.length = ctx->prog_out - ctx->prog_buf;
			my_fwrite(&phoneme_prog_log, 1, sizeof(phoneme_prog_log), ctx->f_prog_log);
		}

		if (compile_phoneme == 0)
			ctx->proc_addr[ctx->n_procs++] =  my_ftell(ctx->f_phindex) / sizeof(unsigned short);
		my_fwrite(ctx->prog_buf, sizeof(unsigned short), ctx->prog_out - ctx->prog_buf, ctx->f_phindex);
	}

	return 0;
}

FLASHMEM static void WritePhonemeTables(CompileContext *ctx)
{
	int ix;
	int j;
	int n;
	...
	my_fputc(value, ctx->f_phtab);
	my_fputc(0, ctx->f_phtab);
	my_fputc(0, ctx->f_phtab);
	my_fputc(0, ctx->f_phtab);
	...
	for (ix = 0; ix < ctx->n_phoneme_tabs; ix++) {
		...
		my_fputc(count+1, ctx->f_phtab);
		my_fputc(ctx->phoneme_tab_list2[ix].includes, ctx->f_phtab);
		my_fputc(0, ctx->f_phtab);
		my_fputc(0, ctx->f_phtab);

		my_fwrite(ctx->phoneme_tab_list2[ix].name, 1, N_PHONEME_TAB_NAME, ctx->f_phtab);
		...
		for (j = 0; j < n; j++) {
			if (p[j].phflags & phLOCAL) {
				...
				p[j].phflags &= ~phLOCAL;
				my_fwrite(&p[j], sizeof(PHONEME_TAB), 1, ctx->f_phtab);
			}
		}
		my_fwrite(&p[n], sizeof(PHONEME_TAB), 1, ctx->f_phtab); // include the extra list-terminator phoneme entry
		my_free(p);
	}
}

FLASHMEM static void EndPhonemeTable(CompileContext *ctx)
{
	int ix;
...
FLASHMEM static void StartPhonemeTable(CompileContext *ctx, const char *name)
{
	int ix;
	int j;
	PHONEME_TAB *p;
	...
	p = (PHONEME_TAB *)my_calloc(sizeof(PHONEME_TAB), N_PHONEME_TAB);
...
FLASHMEM static void CompilePhonemeFiles(CompileContext *ctx)
{
	int item;
	MY_FILE *f;
	...
	for (;;) {
		...
		case kINCLUDE:
			...
			if ((ctx->stack_ix < N_STACK) && (f = my_fopen(buf, "rb")) != NULL) {
                                 ...
                                 espeak_ng_ERROR_CONTEXT *context)
{
	...
	my_fprintf(log, "Compiling phoneme data: %s\n", fname);
	ctx->f_in = my_fopen(fname, "rb");
	if (ctx->f_in == NULL) {
		clean_context(ctx);
	...
	if ((ctx->f_phcontents = my_fopen(fname, "w")) == NULL)
		ctx->f_phcontents = my_stderr;

	my_fprintf(ctx->f_phcontents,
	...
	my_fclose(ctx->f_phdata);
	my_fclose(ctx->f_phindex);
	my_fclose(ctx->f_phtab);
	if (ctx->f_prog_log != NULL)
		my_fclose(ctx->f_prog_log);
	...
	if (ctx->resample_count > 0) {
		...
		my_fprintf(log, "Compiled phonemes: %d errors, %d files resampled to %d Hz.\n", ctx->error_count, ctx->resample_count, samplerate_native);
	} else
		my_fprintf(log, "Compiled phonemes: %d errors.\n", ctx->error_count);

	if (ctx->f_errors != my_stderr && ctx->f_errors != my_stdout)
		my_fclose(ctx->f_errors);
...
PROGMEM static const TUNE default_tune = {
...
PROGMEM static const MNEM_TAB envelope_names[] = {
	{ "fall", 0 },
	{ "rise", 2 },
	{ "fall-rise", 4 },
	{ "fall-rise2", 6 },
...
FLASHMEM static int LookupEnvelopeName(const char *name)
{
...
FLASHMEM espeak_ng_STATUS espeak_ng_CompileIntonation(MY_FILE *log, espeak_ng_ERROR_CONTEXT *context)
{
                                ...
                                )
{
	...
	if ((ctx->f_in = my_fopen(buf, "r")) == NULL) {
	...
	while (!my_feof(ctx->f_in)) {
		if (my_fgets(buf, sizeof(buf), ctx->f_in) == NULL)
			break;
	...
	tune_data = (n_tune_names == 0) ? NULL : (TUNE *)my_calloc(n_tune_names, sizeof(TUNE));
	if (tune_data == NULL) {
		my_fclose(ctx->f_in);
		my_fclose(ctx->f_errors);
		clean_context(ctx);
	...
	my_fwrite(tune_data, n_tune_names, sizeof(TUNE), f_out);
	my_free(tune_data);
	my_fclose(ctx->f_in);
	my_fclose(f_out);

	my_fprintf(log, "Compiled %d intonation tunes: %d errors.\n", n_tune_names, ctx->error_count);
