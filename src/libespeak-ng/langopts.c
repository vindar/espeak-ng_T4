...
FLASHMEM void LoadLanguageOptions(Translator *translator, int key, char *keyValue ) {
				...
				if ((n = atoi(keyValue)) > 0) {
					...
					if (n < 32) {
						...
						if (n < 64)
							...
							my_fprintf(my_stderr, "numbers: Bad option number %d\n", n);					}
				}
            ...
            case V_TUNES: {
                ...
                for (ix = 0; ix < n; ix++) {
                    ...
                    if ((value = LookupTune(names[ix])) < 0)
                        my_fprintf(my_stderr, "Unknown tune '%s'\n", names[ix]);
                    else
...
FLASHMEM static int LookupTune(const char *name) {
	int ix;
...
FLASHMEM int CheckTranslator(Translator *tr, const MNEM_TAB *keyword_tab, int key)
{
	...
	my_fprintf(my_stderr, "Cannot set %s: language not set, or is invalid.\n", LookupMnemName(keyword_tab, key));
	return 1;
}