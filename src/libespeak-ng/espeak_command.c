...
FLASHMEM t_espeak_command *create_espeak_text(const void *text, size_t size, unsigned int position, espeak_POSITION_TYPE position_type, unsigned int end_position, unsigned int flags, void *user_data)
{
	if (!text || !size)
		return NULL;
	...
	t_espeak_command *a_command = (t_espeak_command *)my_malloc(sizeof(t_espeak_command));
	if (!a_command)
		return NULL;

	a_text = my_malloc(size+1);
	if (!a_text) {
		my_free(a_command);
		return NULL;
	}
...
FLASHMEM t_espeak_command *create_espeak_terminated_msg(unsigned int unique_identifier, void *user_data)
{
...
FLASHMEM t_espeak_command *create_espeak_mark(const void *text, size_t size, const char *index_mark, unsigned int end_position, unsigned int flags, void *user_data)
{
...
FLASHMEM t_espeak_command *create_espeak_key(const char *key_name, void *user_data)
{
	if (!key_name)
		return NULL;
...
FLASHMEM t_espeak_command *create_espeak_char(wchar_t character, void *user_data)
{
	t_espeak_command *a_command = (t_espeak_command *)my_malloc(sizeof(t_espeak_command));
	if (!a_command)
		return NULL;
...
FLASHMEM t_espeak_command *create_espeak_parameter(espeak_PARAMETER parameter, int value, int relative)
{
...
FLASHMEM t_espeak_command *create_espeak_punctuation_list(const wchar_t *punctlist)
{
	if (!punctlist)
		return NULL;
	...
	wchar_t *a_list = (wchar_t *)my_malloc(len);
	if (a_list == NULL) {
		my_free(a_command);
		return NULL;
	}
...
FLASHMEM t_espeak_command *create_espeak_voice_name(const char *name)
{
	if (!name)
		return NULL;
...
FLASHMEM t_espeak_command *create_espeak_voice_spec(espeak_VOICE *voice)
{
	if (!voice)
		return NULL;
...
FLASHMEM int delete_espeak_command(t_espeak_command *the_command)
{
	int a_status = 0;
	if (the_command) {
		...
		case ET_TEXT:
			if (the_command->u.my_text.text)
				my_free(the_command->u.my_text.text);
			break;
		case ET_MARK:
			if (the_command->u.my_mark.text)
				my_free(the_command->u.my_mark.text);
			if (the_command->u.my_mark.index_mark)
				my_free((void *)(the_command->u.my_mark.index_mark));
			break;
		...
		case ET_KEY:
			if (the_command->u.my_key.key_name)
				my_free((void *)(the_command->u.my_key.key_name));
			break;
		case ET_CHAR:
		case ET_PARAMETER:
			// No allocation
		...
		case ET_PUNCTUATION_LIST:
			if (the_command->u.my_punctuation_list)
				my_free((void *)(the_command->u.my_punctuation_list));
			break;
		case ET_VOICE_NAME:
			if (the_command->u.my_voice_name)
				my_free((void *)(the_command->u.my_voice_name));
			break;
		case ET_VOICE_SPEC:
		{
			...
			if (data->name)
				my_free((void *)data->name);

			if (data->languages)
				my_free((void *)data->languages);

			if (data->identifier)
				my_free((void *)data->identifier);
		}
			break;
		default:
			assert(0);
...
FLASHMEM void process_espeak_command(t_espeak_command *the_command)
{
	if (the_command == NULL)
		return;
