...
PROGMEM const unsigned char pause_phonemes[8] = {
...
FLASHMEM static int SubstitutePhonemes(PHONEME_LIST *plist_out)
{
...
FLASHMEM void MakePhonemeList(Translator *tr, int post_pause, bool start_sentence)
{
	int ix = 0;
	int j;
	int insert_ph = 0;
	...
	PHONEME_LIST *ph_list3 = my_calloc(1, sizeof(PHONEME_LIST)*N_PHONEME_LIST);
	...
	for (j = 0; insert_ph || ((j < n_ph_list3) && (ix < N_PHONEME_LIST-3)); j++) {
		...
		if ((plist3+1)->synthflags & SFLAG_LENGTHEN) {
			PROGMEM static const char types_double[] = { phFRICATIVE, phVFRICATIVE, phNASAL, phLIQUID, 0 };
