...
static voice_t* pv2 = NULL; // from function static variable v2
static wavegen_peaks_t * peaks = NULL;
static RESONATOR * rbreath = NULL;
static int* hspect[2] = { NULL, NULL};

FLASHMEM int my_init_wavegen(void)
	{
	harm_inc = (int*)my_calloc(1, N_LOWHARM * sizeof(int));
	if (harm_inc == NULL) return 0;

	pv2 = (voice_t*)my_calloc(1, sizeof(voice_t));
	if (pv2 == NULL) return 0;

    wcmdq_bundle = (SSS_wcmdq*)my_calloc(1, sizeof(SSS_wcmdq));
    if (wcmdq_bundle == NULL) return 0;

	peaks = (wavegen_peaks_t*)my_calloc(1, N_PEAKS * sizeof(wavegen_peaks_t));
	if (peaks == NULL) return 0;

	rbreath = (RESONATOR*)my_calloc(1, N_PEAKS * sizeof(RESONATOR));
	if (rbreath == NULL) return 0;

	for (int j = 0; j < 2; j++)
		{
		hspect[j] = (int*)my_calloc(1, MAX_HARMONIC * sizeof(int));
		if (hspect[j] == NULL) return 0;
		}

	return 1;
	}

FLASHMEM void my_free_wavegen(void)
	{
    my_free(harm_inc); harm_inc = NULL;
	my_free(pv2); pv2 = NULL;
	my_free(wcmdq_bundle); wcmdq_bundle = NULL;
    my_free(peaks); peaks = NULL;
    my_free(rbreath); rbreath = NULL;
    for (int j = 0; j < 2; j++) { my_free(hspect[j]); hspect[j] = NULL; }
	}
...
// pitch,speed,
PROGMEM int embedded_default[N_EMBEDDED_VALUES]    = { 0,     50, espeakRATE_NORMAL, 100, 50,  0,  0, 0, espeakRATE_NORMAL, 0, 0, 0, 0, 0, 0 };
PROGMEM static int embedded_max[N_EMBEDDED_VALUES] = { 0, 0x7fff, 750, 300, 99, 99, 99, 0, 750, 0, 0, 0, 0, 4, 0 };
...
PROGMEM static unsigned char modulation_tab[N_ROUGHNESS][8] = {
...
PROGMEM static unsigned char Flutter_tab[N_FLUTTER] = {
...
PROGMEM static unsigned char wavemult[N_WAVEMULT] = {
...
PROGMEM unsigned char pitch_adjust_tab[MAX_PITCH_VALUE+1] = {
...
FLASHMEM int WcmdqUsed()
{
	return N_WCMDQ - WcmdqFree();
}

FLASHMEM void WcmdqInc()
{
	wcmdq_tail++;
	if (wcmdq_tail >= N_WCMDQ) wcmdq_tail = 0;
}

FLASHMEM static void WcmdqIncHead()
{
	MAKE_MEM_UNDEFINED(&(wcmdq_bundle->wcmdq)[wcmdq_head], sizeof(wcmdq_bundle->wcmdq[wcmdq_head]));
	wcmdq_head++;
...
PROGMEM unsigned char pk_shape1[PEAKSHAPEW+1] = {
...
PROGMEM static unsigned char pk_shape2[PEAKSHAPEW+1] = {
...
FLASHMEM void WavegenInit(int rate, int wavemult_fact)
{
	int ix;
	double x;
...
FLASHMEM void WavegenFini(void)
{
#ifdef INCLUDE_KLATT
	KlattFini();
#endif
...
FLASHMEM int GetAmplitude(void)
{
	int amp;
...
FLASHMEM static void WavegenSetEcho(void)
{
	if (wvoice == NULL)
		return;
...
#ifdef ESPEAK_HEAP_HACK
	if (echo_buf==NULL)
		echo_buf = my_malloc(sizeof(short)*N_ECHO_BUF);
	assert(echo_buf!=NULL);
#endif

	memset(echo_buf, 0, sizeof(short) * N_ECHO_BUF);	 // BUGFIX ????
...
FLASHMEM int PeaksToHarmspect(wavegen_peaks_t *peaks, int pitch, int *htab, int control)
{
	if (wvoice == NULL)
		return 1;
...
FLASHMEM static void AdvanceParameters()
{
...
FLASHMEM static double resonator(RESONATOR *r, double input)
{
	double x;
...
FLASHMEM static void setresonator(RESONATOR *rp, int freq, int bwidth, int init)
{
...
FLASHMEM static void SetBreath()
{
	int pk;
...
FLASHMEM static int ApplyBreath(void)
{
	if (wvoice == NULL)
		return 0;
...
FLASHMEM static int Wavegen(int length, int modulation, bool resume, frame_t *fr1, frame_t *fr2, voice_t *wvoice)
{
	if (resume == false)
...
FLASHMEM static int PlaySilence(int length, bool resume)
{
...
FLASHMEM static int PlayWave(int length, bool resume, unsigned char *data, int scale, int amp)
{
	static int n_samples;
	static int ix = 0;
	int value;
...
FLASHMEM static int SetWithRange0(int value, int max)
{
	if (value < 0)
		return 0;
	if (value > max)
...
FLASHMEM static void SetPitchFormants()
{
	if (wvoice == NULL)
		return;
...
FLASHMEM void SetEmbedded(int control, int value)
{
...
FLASHMEM void WavegenSetVoice(voice_t *v)
{
...
FLASHMEM static void SetAmplitude(int length, unsigned char *amp_env, int value)
{
	if (wvoice == NULL)
		return;
...
FLASHMEM void SetPitch2(voice_t *voice, int pitch1, int pitch2, int *pitch_base, int *pitch_range)
{
	int base;
	int range;
	int pitch_value;
...
FLASHMEM static void SetPitch(int length, unsigned char *env, int pitch1, int pitch2)
{
	if (wvoice == NULL)
		return;
...
FLASHMEM static void SetSynth(int length, int modn, frame_t *fr1, frame_t *fr2, voice_t *v)
{
	...
	for (qix = wcmdq_head+1;; qix++) {
		...
		int cmd = wcmdq_bundle->wcmdq[qix][0];
		if (cmd == WCMD_SPECT) {
...
FLASHMEM void Write4Bytes(MY_FILE *f, int value)
{
	// Write 4 bytes to a file, least significant first
	int ix;

	for (ix = 0; ix < 4; ix++) {
		my_fputc(value & 0xff, f);
		value = value >> 8;
	}
}

FLASHMEM static int WavegenFill2()
{
	...
	while (out_ptr < out_end) {
		...
		result = 0;
		q = wcmdq_bundle->wcmdq[wcmdq_head];
		length = q[1];
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS
