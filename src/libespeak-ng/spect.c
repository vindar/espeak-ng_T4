...
PROGMEM static int default_freq[N_PEAKS] =
{ 200, 500, 1200, 3000, 3500, 4000, 6900, 7800, 9000 };
PROGMEM static int default_width[N_PEAKS] =
{ 750, 500, 550, 550, 600, 700, 700, 700, 700 };
PROGMEM static int default_klt_bw[N_PEAKS] =
{ 89, 90, 140, 260, 260, 260, 500, 500, 500 };

FLASHMEM static double read_double(MY_FILE *stream)
{
	unsigned char bytes[10];
	my_fread(bytes, sizeof(char), 10, stream);
	return ieee_extended_to_double(bytes);
}

FLASHMEM float polint(float xa[], float ya[], int n, float x)
{
...
FLASHMEM static SpectFrame *SpectFrameCreate()
{
	int ix;
	SpectFrame *frame;

	frame = my_malloc(sizeof(SpectFrame));
	if (!frame)
		return NULL;
...
FLASHMEM static void SpectFrameDestroy(SpectFrame *frame)
{
	if (frame->spect != NULL)
		my_free(frame->spect);
	my_free(frame);
...
FLASHMEM static espeak_ng_STATUS LoadFrame(SpectFrame *frame, MY_FILE *stream, int file_format_type)
{
	short ix;
	short x;
	...
	my_fread(&frame->nx, sizeof(short), 1, stream);
	my_fread(&frame->markers, sizeof(short), 1, stream);
	my_fread(&frame->amp_adjust, sizeof(short), 1, stream);
	...
	if (file_format_type == 2) {
		my_fread(&ix, sizeof(short), 1, stream); // spare
		my_fread(&ix, sizeof(short), 1, stream); // spare
	}

	for (ix = 0; ix < N_PEAKS; ix++) {
		my_fread(&frame->formants[ix].freq, sizeof(short), 1, stream);
		my_fread(&frame->formants[ix].bandw, sizeof(short), 1, stream);
		my_fread(&frame->peaks[ix].pkfreq, sizeof(short), 1, stream);
		my_fread(&frame->peaks[ix].pkheight, sizeof(short), 1, stream);
		my_fread(&frame->peaks[ix].pkwidth, sizeof(short), 1, stream);
		my_fread(&frame->peaks[ix].pkright, sizeof(short), 1, stream);
		...
		if (file_format_type == 2) {
			my_fread(&frame->peaks[ix].klt_bw, sizeof(short), 1, stream);
			my_fread(&frame->peaks[ix].klt_ap, sizeof(short), 1, stream);
			my_fread(&frame->peaks[ix].klt_bp, sizeof(short), 1, stream);
	...
	if (file_format_type > 0) {
		...
		{
			my_fread(frame->klatt_param + ix, sizeof(short), 1, stream);
			frame->klatt_param[ix] = le16toh(frame->klatt_param[ix]);
		}
	}

	spect_data = my_malloc(sizeof(unsigned short) * frame->nx);
	...
	for (ix = 0; ix < frame->nx; ix++) {
		my_fread(&x, sizeof(short), 1, stream);
		x = le16toh(x);
		spect_data[ix] = x;
...
FLASHMEM double GetFrameRms(SpectFrame *frame, int seq_amplitude)
{
	int h;
	float total = 0;
	int maxh;
	...
	int* htab = my_calloc(1, sizeof(int)*400);
	assert(htab!=NULL);
	wavegen_peaks_t* wpeaks = my_calloc(1, sizeof(wavegen_peaks_t)*9);
	assert(wpeaks!=NULL);
#else
	int htab[400];
...
FLASHMEM SpectSeq *SpectSeqCreate()
{
	SpectSeq *spect = my_malloc(sizeof(SpectSeq));
	if (!spect)
		return NULL;
...
FLASHMEM void SpectSeqDestroy(SpectSeq *spect)
{
	int ix;
...
FLASHMEM static float GetFrameLength(SpectSeq *spect, int frame)
{
	int ix;
	float adjust = 0;
...
#pragma GCC visibility push(default)
FLASHMEM espeak_ng_STATUS LoadSpectSeq(SpectSeq *spect, const char *filename)
{
	short n, temp;
	int ix;
	...
	MY_FILE *stream = my_fopen(filename, "rb");
	if (stream == NULL) {
		my_fprintf(my_stderr, "Failed to open: '%s'", filename);
		return errno;
	}

	my_fread(&id1, sizeof(uint32_t), 1, stream);
	id1 = le32toh(id1);
	my_fread(&id2, sizeof(uint32_t), 1, stream);
	id2 = le32toh(id2);
	...
	if ((id1 == FILEID1_SPECTSEQ) && (id2 == FILEID2_SPECTSEQ))
		...
		my_fprintf(my_stderr, "Unsupported spectral file format.\n");
		my_fclose(stream);
		return ENS_UNSUPPORTED_PHON_FORMAT;
	}

	my_fread(&name_len, sizeof(uint32_t), 1, stream);
	name_len = le32toh(name_len);
	if (name_len > 0) {
		if ((spect->name = (char *)my_malloc(name_len)) == NULL) {
			my_fclose(stream);
			return ENOMEM;
		}
		my_fread(spect->name, sizeof(char), name_len, stream);
	} else
		spect->name = NULL;

	my_fread(&n, sizeof(short), 1, stream);
	my_fread(&spect->amplitude, sizeof(short), 1, stream);
	my_fread(&spect->max_y, sizeof(short), 1, stream);
	my_fread(&temp, sizeof(short), 1, stream); // unused
	n = le16toh(n);
	...
	spect->frames = my_calloc(n, sizeof(SpectFrame *));
