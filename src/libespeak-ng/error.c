                          ...
                          const char *filename)
{
	if (context) {
		if (*context) {
			my_free((*context)->name);
		} else {
			*context = my_malloc(sizeof(espeak_ng_ERROR_CONTEXT_));
			if (!*context)
				return ENOMEM;
		}
                                 ...
                                 espeak_ng_ERROR_CONTEXT context)
{
	...
	if (context) {
		...
		case ERROR_CONTEXT_FILE:
			my_fprintf(out, "Error processing file '%s': %s.\n", context->name, error);
			break;
		case ERROR_CONTEXT_VERSION:
			my_fprintf(out, "Error: %s at '%s' (expected 0x%x, got 0x%x).\n",
			        error, context->name, context->expected_version, context->version);
			break;
		}
	} else
		my_fprintf(out, "Error: %s.\n", error);
}
