...
MY_FILE *f_trans = NULL; // phoneme output text
...
PROGMEM static const unsigned short breaks[] = { '_', 0 };
...
static char* source = NULL; // [N_TR_SOURCE + 40]; // extra space for embedded command & voice change info at end
PHONEME_LIST2* ph_list2 = NULL; // first stage of text->phonemes
REPLACE_PHONEMES * replace_phonemes = NULL;
unsigned int * embedded_list = NULL;
wchar_t * option_punctlist = NULL;
char * word_phonemes = NULL; // a word translated into phoneme codes

FLASHMEM int my_init_ph_list2(void)
	{
	source = (char*)my_calloc(1,  (N_TR_SOURCE+40)*sizeof(char));
	if (source == NULL) return 0;

	ph_list2 = (PHONEME_LIST2*)my_calloc(1, N_PHONEME_LIST * sizeof(PHONEME_LIST2));
    if (ph_list2 == NULL) return 0;

	replace_phonemes = (REPLACE_PHONEMES*)my_calloc(1, N_REPLACE_PHONEMES * sizeof(REPLACE_PHONEMES));
	if (replace_phonemes == NULL) return 0;

	embedded_list = (unsigned int*)my_calloc(1, N_EMBEDDED_LIST * sizeof(unsigned int));
	if (embedded_list == NULL) return 0;

	option_punctlist = (wchar_t*)my_calloc(1, N_PUNCTLIST * sizeof(wchar_t));
	if (option_punctlist == NULL) return 0;

	word_phonemes = (char*)my_calloc(1, N_WORD_PHONEMES * sizeof(char));
	if (word_phonemes == NULL) return 0;

	return 1;
	}

FLASHMEM void my_free_ph_list2(void)
	{
	my_free(source); source = NULL;
	my_free(ph_list2); ph_list2 = NULL;
    my_free(replace_phonemes); replace_phonemes = NULL;
    my_free(embedded_list); embedded_list = NULL;
    my_free(option_punctlist); option_punctlist = NULL;
    my_free(word_phonemes); word_phonemes = NULL;
	}
...
FLASHMEM void DeleteTranslator(Translator *tr)
{
	if (!tr) return;
...
FLASHMEM int lookupwchar(const unsigned short *list, int c)
{
...
FLASHMEM char *strchr_w(const char *s, int c)
	{
...
FLASHMEM int TranslateWord(Translator *tr, char *word_start, WORD_TAB *wtab, char *word_out)
	{
	char words_phonemes[N_WORD_PHONEMES]; // a word translated into phoneme codes
	char *phonemes = words_phonemes;


	int flags = TranslateWord3(tr, word_start, wtab, word_out, &any_stressed_words, current_alphabet, word_phonemes, N_WORD_PHONEMES * sizeof(char));
	if (flags & FLAG_TEXTMODE && word_out) {
		...
		while (*word_out && available > 1) {
			...
			TranslateWord3(tr, word_out, wtab, NULL, &any_stressed_words, current_alphabet, word_phonemes, N_WORD_PHONEMES * sizeof(char));
		...
		if (phonemes != words_phonemes) {
			snprintf(word_phonemes, N_WORD_PHONEMES * sizeof(char), "%s", words_phonemes);
		}
	}
	return flags;
}

FLASHMEM static void SetPlist2(PHONEME_LIST2 *p, unsigned char phcode)
{
	p->phcode = phcode;
	p->stresslevel = 0;
	p->tone_ph = 0;
...
FLASHMEM static int CountSyllables(unsigned char *phonemes)
{
	int count = 0;
	int phon;
...
FLASHMEM static void Word_EmbeddedCmd()
{
...
FLASHMEM static int SetAlternateTranslator(const char *new_language, Translator **translator, char translator_language[20])
{
...
FLASHMEM int SetTranslator2(const char *new_language)
{
	return SetAlternateTranslator(new_language, &translator2, translator2_language);
}

FLASHMEM int SetTranslator3(const char *new_language)
{
	return SetAlternateTranslator(new_language, &translator3, translator3_language);
}

FLASHMEM static int TranslateWord2(Translator *tr, char *word, WORD_TAB *wtab, int pre_pause)
{
	int flags = 0;
	int stress;
	int next_stress;
...
FLASHMEM static int EmbeddedCommand(unsigned int *source_index_out)
{
...
FLASHMEM static const char *FindReplacementChars(Translator *tr, const char **pfrom, unsigned int c, const char *next, int *ignore_next_n)
{
...
FLASHMEM static int SubstituteChar(Translator *tr, unsigned int c, unsigned int next_in, const char *next, int *insert, int *wordflags)
{
...
FLASHMEM static int TranslateChar(Translator *tr, char *ptr, int prev_in, unsigned int c, unsigned int next_in, int *insert, int *wordflags)
{
	...
	PROGMEM static const unsigned char hangul_compatibility[0x34] = {
...
FLASHMEM static int UpperCaseInWord(Translator *tr, char *word, int c)
{
...
FLASHMEM void TranslateClause(Translator *tr, int *tone_out, char **voice_change)
{
	...
	WORD_TAB* words = my_calloc(1, sizeof(WORD_TAB)*N_CLAUSE_WORDS);
	assert(words!=NULL);

	WORD_TAB* num_wtab = my_calloc(1, sizeof(WORD_TAB)*50);
...
FLASHMEM static int CalcWordLength(int source_index, int charix_top, short int *charix, WORD_TAB *words, int word_count) {
	int j;
	int k;
...
FLASHMEM static void CombineFlag(Translator *tr, WORD_TAB *wtab, char *word, int *flags, unsigned char *p, char *word_phonemes) {
...
FLASHMEM static void SwitchLanguage(char *word, char *word_phonemes) {
	char lang_name[12];
	int ix;
...
FLASHMEM void InitText(int control)
{
	count_sentences = 0;
	count_words = 0;
