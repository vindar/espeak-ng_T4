...
static SSML_STACK * ssml_stack = NULL;
PARAM_STACK * param_stack = NULL;

FLASHMEM int my_init_ssml_param_stack(void)
	{
	ssml_stack = (SSML_STACK*)my_calloc(1, N_SSML_STACK * sizeof(SSML_STACK));
	if (!ssml_stack) return 0; 
	param_stack = (PARAM_STACK*)my_calloc(1, N_PARAM_STACK * sizeof(PARAM_STACK));
	if (!param_stack) return 0;
	return 1; 
	}

FLASHMEM void my_free_ssml_param_stack(void)
	{
    my_free(ssml_stack); ssml_stack = NULL;
    my_free(param_stack); param_stack = NULL;
	}
...
FLASHMEM int clause_type_from_codepoint(uint32_t c)
{
...
FLASHMEM static int IsRomanU(unsigned int c)
{
	if ((c == 'I') || (c == 'V') || (c == 'X') || (c == 'L'))
		return 1;
	return 0;
...
FLASHMEM const char *WordToString2(unsigned int word)
{
...
FLASHMEM static const char *LookupSpecial(Translator *tr, const char *string, char *text_out)
{
...
FLASHMEM static const char *LookupCharName(Translator *tr, int c, bool only)
{
...
FLASHMEM static int AnnouncePunctuation(Translator *tr, int c1, int *c2_ptr, char *output, int *bufix, int end_clause)
{
...
FLASHMEM int AddNameData(const char *name, int wide)
{
	...
	if (namedata_ix+len >= n_namedata) {
		...
		void *vp;
		if ((vp = my_realloc(namedata, namedata_ix+len + 1000)) == NULL)
...
FLASHMEM void SetVoiceStack(espeak_VOICE *v, const char *variant_name)
{
	SSML_STACK *sp;
	sp = &ssml_stack[0];
...
FLASHMEM static void RemoveChar(char *p)
{
...
FLASHMEM static int lookupwchar2(const unsigned short *list, int c)
{
...
FLASHMEM static bool IgnoreOrReplaceChar(Translator *tr, int *c1) {
    int i;
...
FLASHMEM static int CheckPhonemeMode(int option_phoneme_input, int phoneme_mode, int c1, int c2) {
...
FLASHMEM int ReadClause(Translator *tr, char *buf, short *charix, int *charix_top, int n_buf, int *tone_type, char *voice_change)
{
...
FLASHMEM void InitNamedata(void)
{
	namedata_ix = 0;
	if (namedata != NULL) {
		my_free(namedata);
...
FLASHMEM static void TerminateBufWithSpaceAndZero(char *buf, int index, int *ungetc) {
	buf[index] = ' ';
	buf[index+1] = 0;
...
FLASHMEM static void DecodeWithPhonemeMode(char *buf, char *phonemes, Translator *tr, Translator *tr2, unsigned int flags[]) {
	char phonemes2[55];
	if (tr2 == NULL) {
