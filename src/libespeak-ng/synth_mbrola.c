...
FLASHMEM espeak_ng_STATUS LoadMbrolaTable(const char *mbrola_voice, const char *phtrans, int *srate)
{
	...
	if (GetFileLength(path) <= 0) {
		...
		if (GetFileLength(path) <= 0) {
			...
			// Show error message
			if (GetFileLength(path) <= 0) {
				my_fprintf(my_stderr, "Cannot find MBROLA voice file '%s' in neither of paths:\n"
						" - /usr/share/mbrola/%s\n"
	...
	if (size < 0) // size == -errno
		return -size;
	if ((f_in = my_fopen(path, "rb")) == NULL) {
		int error = errno;
		close_MBR();
		return error;
	}

	MBROLA_TAB *new_mbrola_tab = (MBROLA_TAB *)my_realloc(mbrola_tab, size);
...
FLASHMEM static int GetMbrName(PHONEME_LIST *plist, PHONEME_TAB *ph, PHONEME_TAB *ph_prev, PHONEME_TAB *ph_next, int *name2, int *split, int *control)
{
...
FLASHMEM static char *WritePitch(int env, int pitch1, int pitch2, int split, int final)
{
...
FLASHMEM int MbrolaTranslate(PHONEME_LIST *plist, int n_phonemes, bool resume, MY_FILE *f_mbrola)
{
	...
	while (phix < n_phonemes) {
		...
		if (f_mbrola)
			my_fwrite(mbr_buf, 1, (ptr-mbr_buf), f_mbrola); // write .pho to a file
		else {
			...
			if (res == 0)
				return 1;
			wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_MBROLA_DATA;
	...
	if (!f_mbrola) {
		...
		wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_MBROLA_DATA;
		wcmdq_bundle->wcmdq[wcmdq_tail][1] = 500;
		WcmdqInc();
	}
...
FLASHMEM int MbrolaGenerate(PHONEME_LIST *phoneme_list, int *n_ph, bool resume)
{
	MY_FILE *f_mbrola = NULL;
...
FLASHMEM int MbrolaFill(int length, bool resume, int amplitude)
{
...
FLASHMEM void MbrolaReset(void)
{
