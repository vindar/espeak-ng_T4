...
FLASHMEM const char *WordToString(unsigned int word)
{
...
FLASHMEM void SynthesizeInit()
{
#ifdef ESPEAK_HEAP_HACK
    if (phoneme_list==NULL)
		phoneme_list = my_malloc(sizeof(PHONEME_LIST)*(N_PHONEME_LIST+1));
#endif
...
FLASHMEM static void EndAmplitude(void)
{
	if (amp_length > 0) {
		if (wcmdq_bundle->wcmdq[last_amp_cmd][1] == 0)
			wcmdq_bundle->wcmdq[last_amp_cmd][1] = amp_length;
		amp_length = 0;
	}
}

FLASHMEM static void EndPitch(int voice_break)
{
	// possible end of pitch envelope, fill in the length
	if ((pitch_length > 0) && (last_pitch_cmd >= 0)) {
		if (wcmdq_bundle->wcmdq[last_pitch_cmd][1] == 0)
			wcmdq_bundle->wcmdq[last_pitch_cmd][1] = pitch_length;
		pitch_length = 0;
	}
...
FLASHMEM static void DoAmplitude(int amp, unsigned char *amp_env)
{
	intptr_t *q;
	...
	q = wcmdq_bundle->wcmdq[wcmdq_tail];
	q[0] = WCMD_AMPLITUDE;
...
FLASHMEM static void DoPhonemeAlignment(char* pho, int type)
{
	wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_PHONEME_ALIGNMENT;
	wcmdq_bundle->wcmdq[wcmdq_tail][1] = (intptr_t)pho;
	wcmdq_bundle->wcmdq[wcmdq_tail][2] = type;
	WcmdqInc();
}

FLASHMEM static void DoPitch(unsigned char *env, int pitch1, int pitch2)
{
	intptr_t *q;
...
FLASHMEM int PauseLength(int pause, int control)
{
	unsigned int len;
...
FLASHMEM static void DoPause(int length, int control)
{
	...
	EndPitch(1);
	wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_PAUSE;
	wcmdq_bundle->wcmdq[wcmdq_tail][1] = len;
	WcmdqInc();
	last_frame = NULL;

	if (fmt_amplitude != 0) {
		wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_FMT_AMPLITUDE;
		wcmdq_bundle->wcmdq[wcmdq_tail][1] = fmt_amplitude = 0;
		WcmdqInc();
	}
}
...
FLASHMEM static int DoSample2(int index, int which, int std_length, int control, int length_mod, int amp)
{
	int length;
	int wav_length;
	int wav_scale;
...
FLASHMEM int DoSample3(PHONEME_DATA *phdata, int length_mod, int amp)
{
	int amp2;
	int len;
	EndPitch(1);
...
static frame_t* frame_pool_pttr = NULL;

FLASHMEM int my_init_frame_pool_pttr(void)
	{
    frame_pool_pttr = (frame_t*)my_calloc(1, N_WCMDQ * sizeof(frame_t));
    if (frame_pool_pttr == NULL) return 0;
	return 1; 
	}

FLASHMEM void my_free_frame_pool_pttr(void)
	{
    my_free(frame_pool_pttr);
	frame_pool_pttr = NULL;
	}

FLASHMEM static frame_t *AllocFrame()
{
	...
	MAKE_MEM_UNDEFINED(&frame_pool_pttr[ix], sizeof(frame_pool_pttr[ix]));
	return &(frame_pool_pttr[ix]);
}

FLASHMEM static void set_frame_rms(frame_t *fr, int new_rms)
{
	...
	PROGMEM static const short sqrt_tab[200] = {
...
FLASHMEM static void formants_reduce_hf(frame_t *fr, int level)
{
...
FLASHMEM static frame_t *CopyFrame(frame_t *frame1, int copy)
{
...
FLASHMEM static frame_t *DuplicateLastFrame(frameref_t *seq, int n_frames, int length)
{
	frame_t *fr;
...
FLASHMEM static void AdjustFormants(frame_t *fr, int target, int min, int max, int f1_adj, int f3_adj, int hf_reduce, int flags)
{
	int x;
...
FLASHMEM static int VowelCloseness(frame_t *fr)
{
...
FLASHMEM int FormantTransition2(frameref_t *seq, int *n_frames, unsigned int data1, unsigned int data2, PHONEME_TAB *other_ph, int which)
{
	int len;
	int rms;
	int f1;
	...
	const short vcolouring[N_VCOLOUR][5] = {
...
FLASHMEM static void SmoothSpect(void)
{
	...
	q = wcmdq_bundle->wcmdq[syllable_centre];
...
FLASHMEM static void StartSyllable(void)
{
...
FLASHMEM int DoSpect2(PHONEME_TAB *this_ph, int which, FMT_PARAMS *fmt_params,  PHONEME_LIST *plist, int modulation)
{
	...
	if (last_frame != NULL) {
		if (((last_frame->length < 2) || (last_frame->frflags & FRFLAG_VOWEL_CENTRE))
			...
			wcmdq_bundle->wcmdq[last_wcmdq][3] = (intptr_t)frame1;
			...
			if (last_frame->frflags & FRFLAG_BREAK_LF) {
				...
				wcmdq_bundle->wcmdq[last_wcmdq][3] = (intptr_t)fr;
			}
		}
	}
...
FLASHMEM void DoMarker(int type, int char_posn, int length, int value)
{
	...
	if (WcmdqFree() > 5) {
		wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_MARKER + (type << 8);
		wcmdq_bundle->wcmdq[wcmdq_tail][1] = (char_posn & 0xffffff) | (length << 24);
		wcmdq_bundle->wcmdq[wcmdq_tail][2] = value;
		WcmdqInc();
	}
}

FLASHMEM void DoPhonemeMarker(int type, int char_posn, int length, char *name)
{
	// This could be used to return an index to the word currently being spoken
	// Type 7=phoneme
	...
	if (WcmdqFree() > 5) {
		wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_MARKER + (type << 8);
		wcmdq_bundle->wcmdq[wcmdq_tail][1] = (char_posn & 0xffffff) | (length << 24);
		memcpy(&(wcmdq_bundle->wcmdq)[wcmdq_tail][2], name, 8); // up to 8 bytes of UTF8 characters
		WcmdqInc();
	}
}
...
void DoSonicSpeed(int value)
{
	...
	wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_SONIC_SPEED;
	wcmdq_bundle->wcmdq[wcmdq_tail][1] = value;
	WcmdqInc();
}
#endif

FLASHMEM espeak_ng_STATUS DoVoiceChange(voice_t *v)
{
	// allocate memory for a copy of the voice data, and free it in wavegenfill()
	voice_t *v2;
	if ((v2 = (voice_t *)my_malloc(sizeof(voice_t))) == NULL)
		return ENOMEM;
	memcpy(v2, v, sizeof(voice_t));
	wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_VOICE;
	wcmdq_bundle->wcmdq[wcmdq_tail][2] = (intptr_t)v2;
	WcmdqInc();
	return ENS_OK;
}

FLASHMEM void DoEmbedded(int *embix, int sourceix)
{
	...
	do {
		...
		case EMBED_I: // play dynamically loaded wav data (sound icon)
			if ((int)value < n_soundicon_tab) {
				if (soundicon_tab[value].length != 0) {
					...
					wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_WAVE;
					wcmdq_bundle->wcmdq[wcmdq_tail][1] = soundicon_tab[value].length;
					wcmdq_bundle->wcmdq[wcmdq_tail][2] = (intptr_t)soundicon_tab[value].data + 44; // skip WAV header
					wcmdq_bundle->wcmdq[wcmdq_tail][3] = 0x1500; // 16 bit data, amp=21
					WcmdqInc();
				}
			}
			break;
		...
		default:
			DoPause(10, 0); // ensure a break in the speech
			wcmdq_bundle->wcmdq[wcmdq_tail][0] = WCMD_EMBEDDED;
			wcmdq_bundle->wcmdq[wcmdq_tail][1] = command;
			wcmdq_bundle->wcmdq[wcmdq_tail][2] = value;
			WcmdqInc();
			break;
		}
...
FLASHMEM int Generate(PHONEME_LIST *phoneme_list, int *n_ph, bool resume)
{
	static int ix;
	...
	while ((ix < (*n_ph)) && (ix < N_PHONEME_LIST-2)) {
		...
		if (WcmdqFree() <= free_min) return 1; // wait
...
FLASHMEM int SpeakNextClause(int control)
{
