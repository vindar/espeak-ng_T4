...
static frameref_t * frames_buf = NULL; // [N_SEQ_FRAMES] ; // from static variable in LookupSpect()
PHONEME_TAB* * phoneme_tab = NULL; // [N_PHONEME_TAB] ;
PHONEME_TAB_LIST * phoneme_tab_list = NULL; // N_PHONEME_TABS];

FLASHMEM int my_init_phoneme_tab_phoneme_tab_list(void)
	{
	frames_buf = (frameref_t*)my_calloc(1, N_SEQ_FRAMES * sizeof(frameref_t));
	if (!frames_buf) return 0;

	phoneme_tab = (PHONEME_TAB**)my_calloc(1, N_PHONEME_TAB * sizeof(PHONEME_TAB*));
	if (!phoneme_tab) return 0;
	phoneme_tab_list = (PHONEME_TAB_LIST*)my_calloc(1, N_PHONEME_TABS * sizeof(PHONEME_TAB_LIST));
	if (!phoneme_tab_list) return 0;
	return 1;
	}

FLASHMEM void my_free_phoneme_tab_phoneme_tab_list(void)
	{
    my_free(frames_buf); frames_buf = NULL;
    my_free(phoneme_tab); phoneme_tab = NULL;
    my_free(phoneme_tab_list); phoneme_tab_list = NULL;
	}
...
FLASHMEM static espeak_ng_STATUS ReadPhFile(void **ptr, const char *fname, int *size, espeak_ng_ERROR_CONTEXT *context)
{
	...
	if (ptmp!=NULL){
		...
		*ptr = ptmp;
		...
		if ((f_in = my_fopen(buf, "rb")) == NULL)
			return create_file_error_context(context, errno, buf);

		if (*ptr != NULL)
			my_free(*ptr);

		if ((*ptr = my_malloc(length)) == NULL) {
			my_fclose(f_in);
			return ENOMEM;
		}
		if (my_fread(*ptr, 1, length, f_in) != length) {
			int error = errno;
			my_fclose(f_in);
			my_free(*ptr);
...
FLASHMEM espeak_ng_STATUS LoadPhData(int *srate, espeak_ng_ERROR_CONTEXT *context)
{
	int ix;
	int version;
	int length = 0;
...
FLASHMEM void FreePhData(void)
{
	my_free(phoneme_tab_data);
	my_free(phoneme_index);
	my_free(phondata_ptr);
	my_free(tunes);
...
FLASHMEM int PhonemeCode(unsigned int mnem)
{
	int ix;
...
FLASHMEM int LookupPhonemeString(const char *string)
{
	int ix;
	unsigned int mnem;
...
FLASHMEM frameref_t *LookupSpect(PHONEME_TAB *this_ph, int which, FMT_PARAMS *fmt_params,  int *n_frames, PHONEME_LIST *plist)
{
	int ix;
	int nf;
	int nf1;
...
FLASHMEM unsigned char *GetEnvelope(int index)
{
	if (index == 0) {
		my_fprintf(my_stderr, "espeak: No envelope\n");
		return envelope_data[0]; // not found, use a default envelope
	}
	return (unsigned char *)&phondata_ptr[index];
}

FLASHMEM static void SetUpPhonemeTable(int number)
{
	int ix;
	int includes;
	PHONEME_TAB *phtab;
...
FLASHMEM void SelectPhonemeTable(int number)
{
	n_phoneme_tab = 0;
...
FLASHMEM int LookupPhonemeTable(const char *name)
{
	int ix;
...
FLASHMEM int SelectPhonemeTableName(const char *name)
{
...
FLASHMEM static void InvalidInstn(PHONEME_TAB *ph, int instn)
{
	my_fprintf(my_stderr, "Invalid instruction %.4x for phoneme '%s'\n", instn, WordToString(ph->mnemonic));
}

FLASHMEM static bool StressCondition(Translator *tr, PHONEME_LIST *plist, int condition, int control)
{
	int stress_level;
	PHONEME_LIST *pl;
	PROGMEM static const int condition_level[4] = { 1, 2, 4, 15 };
...
FLASHMEM static int CountVowelPosition(PHONEME_LIST *plist)
{
	int count = 0;
...
FLASHMEM static bool InterpretCondition(Translator *tr, int control, PHONEME_LIST *plist, unsigned short *p_prog, WORD_PH_DATA *worddata)
{
	unsigned int data;
	int instn;
	int instn2;
...
FLASHMEM static void SwitchOnVowelType(PHONEME_LIST *plist, PHONEME_DATA *phdata, unsigned short **p_prog, int instn_type)
{
	int voweltype;
...
FLASHMEM static int NumInstnWords(unsigned short *prog)
{
	int instn;
	int instn2;
	int instn_type;
	int n;
	int type2;
	PROGMEM static const char n_words[16] = { 0, 1, 0, 0, 1, 1, 0, 1, 1, 2, 4, 0, 0, 0, 0, 0 };
...
FLASHMEM void InterpretPhoneme(Translator *tr, int control, PHONEME_LIST *plist, PHONEME_DATA *phdata, WORD_PH_DATA *worddata)
{
	// control:
	// bit 0:  PreVoicing
...
FLASHMEM void InterpretPhoneme2(int phcode, PHONEME_DATA *phdata)
{
