...
FLASHMEM void cancel_audio(void)
{
#ifdef HAVE_PCAUDIOLIB_AUDIO_H
...
FLASHMEM static int dispatch_audio(short *outbuf, int length, espeak_EVENT *event)
{
...
FLASHMEM static int create_events(short *outbuf, int length, espeak_EVENT *event_list)
{
	int finished;
	int i = 0;
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_InitializeOutput(espeak_ng_OUTPUT_MODE output_mode, int buffer_length, const char *device)
{
	...
	out_start = (unsigned char *)my_realloc(outbuf, outbuf_size);
	if (out_start == NULL)
		return ENOMEM;
	else
		outbuf = out_start;
	...
	espeak_EVENT *new_event_list = (espeak_EVENT *)my_realloc(event_list, sizeof(espeak_EVENT) * n_event_list);
...
FLASHMEM ESPEAK_NG_API void espeak_ng_InitializePath(const char *path)
{
	//if (check_data_path(path, 1))	return;
	if (path == NULL) { path_home[0] = 0; return;  }
	strcpy(path_home, path);
}

PROGMEM const int param_defaults[N_SPEECH_PARAM] = {
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_Initialize(espeak_ng_ERROR_CONTEXT *context)
{
	int param;
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_SetPhonemeEvents(int enable, int ipa) {
	option_phoneme_events = 0;
	if (enable) {
...
FLASHMEM ESPEAK_NG_API int espeak_ng_GetSampleRate(void)
{
	return samplerate;
}
...
FLASHMEM FLASHMEM static espeak_ng_STATUS Synthesize(unsigned int unique_identifier, const void *text, int flags)
{
...
FLASHMEM void MarkerEvent(int type, unsigned int char_position, int value, int value2, unsigned char *out_ptr)
{
...
FLASHMEM espeak_ng_STATUS sync_espeak_Synth(unsigned int unique_identifier, const void *text,
...
FLASHMEM espeak_ng_STATUS sync_espeak_Synth_Mark(unsigned int unique_identifier, const void *text,
...
FLASHMEM espeak_ng_STATUS sync_espeak_Key(const char *key)
{
...
FLASHMEM espeak_ng_STATUS sync_espeak_Char(wchar_t character)
{
...
FLASHMEM void sync_espeak_SetPunctuationList(const wchar_t *punctlist)
{
...
FLASHMEM ESPEAK_API void espeak_SetSynthCallback(t_espeak_callback *SynthCallback)
{
...
FLASHMEM ESPEAK_API void espeak_SetUriCallback(int (*UriCallback)(int, const char *, const char *))
{
	uri_callback = UriCallback;
}

FLASHMEM ESPEAK_API void espeak_SetPhonemeCallback(int (*PhonemeCallback)(const char *))
{
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_SpeakKeyName(const char *key_name)
{
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_SpeakCharacter(wchar_t character)
{
	// is there a system resource of character names per language?
...
FLASHMEM ESPEAK_API int espeak_GetParameter(espeak_PARAMETER parameter, int current)
{
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_SetParameter(espeak_PARAMETER parameter, int value, int relative)
{
#ifdef USE_ASYNC
	if (my_mode & ENOUTPUT_MODE_SYNCHRONOUS)
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_SetPunctuationList(const wchar_t *punctlist)
{
	// Set the list of punctuation which are spoken for "some".
...
FLASHMEM ESPEAK_API void espeak_SetPhonemeTrace(int phonememode, MY_FILE *stream)
{
	/* phonememode:  Controls the output of phoneme symbols for the text
	   ...
	   stream   output stream for the phoneme symbols (and trace).  If stream=NULL then it uses my_stdout.
	*/
...
FLASHMEM ESPEAK_API const char *espeak_TextToPhonemes(const void **textptr, int textmode, int phonememode)
{
	/* phoneme_mode
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_Cancel(void)
{
#ifdef USE_ASYNC
	fifo_stop();
	event_clear_all();
...
FLASHMEM ESPEAK_API int espeak_IsPlaying(void)
{
#ifdef USE_ASYNC
	return fifo_is_busy();
#else
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_Synchronize(void)
{
...
FLASHMEM ESPEAK_NG_API espeak_ng_STATUS espeak_ng_Terminate(void)
{
#ifdef USE_ASYNC
	fifo_stop();
	fifo_terminate();
...
FLASHMEM ESPEAK_API const char *espeak_Info(const char **ptr)
{
	if (ptr != NULL)
		*ptr = path_home;
