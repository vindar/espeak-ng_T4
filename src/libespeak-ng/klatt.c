...
PROGMEM static const int scale_wav_tab[] = { 45, 38, 45, 45, 55, 45 }; // scale output from different voicing sources
...
FLASHMEM static double resonator(resonator_ptr r, double input)
{
	double x;
...
FLASHMEM static double antiresonator(resonator_ptr r, double input)
{
...
FLASHMEM static void flutter(klatt_frame_ptr frame)
{
...
FLASHMEM static double sampled_source(int source_num)
{
	int itemp;
	double ftemp;
	double result;
...
FLASHMEM static int parwave(klatt_frame_ptr frame, WGEN_DATA *wdata)
{
	double temp;
	int value;
	double outbypas;
...
FLASHMEM void KlattReset(int control)
{
	int r_ix;
...
FLASHMEM static void frame_init(klatt_frame_ptr frame)
{
	double amp_par[7];
	PROGMEM static const double amp_par_factor[7] = { 0.6, 0.4, 0.15, 0.06, 0.04, 0.022, 0.03 };
	long Gain0_tmp;
	int ix;
...
FLASHMEM static double impulsive_source()
{
	PROGMEM static const double doublet[] = { 0.0, 13000000.0, -13000000.0 };
	static double vwave;
...
FLASHMEM static double natural_source()
{
	double lgtemp;
	static double vwave;
...
FLASHMEM static void pitch_synch_par_reset(klatt_frame_ptr frame)
{
	long temp;
	double temp1;
	static long skew;
	PROGMEM static const short B0[224] = {
...
FLASHMEM static void setabc(long int f, long int bw, resonator_ptr rp)
{
	double r;
	double arg;
...
FLASHMEM static void setzeroabc(long int f, long int bw, resonator_ptr rp)
{
	double r;
	double arg;
...
FLASHMEM static double gen_noise(double noise)
{
	long temp;
	static double nlast;
...
FLASHMEM static double DBtoLIN(long dB)
{

	static const short hhamptable[88] = {
	...
	return (double)(hhamptable[dB]) * 0.001;
}
...
FLASHMEM int Wavegen_Klatt(int length, int resume, frame_t *fr1, frame_t *fr2, WGEN_DATA *wdata, voice_t *wvoice)
{
#ifdef INCLUDE_SPEECHPLAYER
...
FLASHMEM static void SetSynth_Klatt(int length, frame_t *fr1, frame_t *fr2, voice_t *wvoice, int control)
{
	int ix;
	double next;
	int qix;
	...
	if (control & 1) {
		...
		for (qix = wcmdq_head+1;; qix++) {
			...
			cmd = wcmdq_bundle->wcmdq[qix][0];
			if (cmd == WCMD_KLATT) {
				end_wave = 0; // next wave generation is from another spectrum

				fr3 = (frame_t *)wcmdq_bundle->wcmdq[qix][2];
				for (ix = 1; ix < 6; ix++) {
...
FLASHMEM void KlattInit()
{

	PROGMEM static const short formant_hz[10] = { 280, 688, 1064, 2806, 3260, 3700, 6500, 7000, 8000, 280 };
	PROGMEM static const short bandwidth[10] = { 89, 160, 70, 160, 200, 200, 500, 500, 500, 89 };
	PROGMEM static const short parallel_amp[10] = { 0, 59, 59, 59, 59, 59, 59, 0, 0, 0 };
	PROGMEM static const short parallel_bw[10] = { 59, 59, 89, 149, 200, 200, 500, 0, 0, 0 };
