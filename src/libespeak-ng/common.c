...
// On ESP32 and possibly other toolchains the wide char versions of isalpha() appear to be broken, do not
// include wctypes and use UCD functions instead
// #include <wctype.h>
...
FLASHMEM int GetFileLength(const char *filename)
{
	return my_getfilelength(filename);
}

FLASHMEM void strncpy0(char *to, const char *from, int size)
{
...
FLASHMEM int utf8_in(int *c, const char *buf)
{
...
FLASHMEM int utf8_out(unsigned int c, char *buf)
{
	...
	int j;
	int shift;
	PROGMEM static const char unsigned code[4] = { 0, 0xc0, 0xe0, 0xf0 };
...
FLASHMEM int utf8_in2(int *c, const char *buf, int backwards)
{
	...
	int c1;
	int n_bytes;
	PROGMEM static const unsigned char mask[4] = { 0xff, 0x1f, 0x0f, 0x07 };
...
FLASHMEM int IsDigit09(unsigned int c)
{
	if ((c >= '0') && (c <= '9'))
		return 1;
	return 0;
...
FLASHMEM int IsDigit(unsigned int c)
{
	if (isdigit(c))
		return 1;
...
FLASHMEM int IsSpace(unsigned int c)
{
	if (c == 0)
		return 0;
...
FLASHMEM int isspace2(unsigned int c)
{
...
FLASHMEM int is_str_totally_null(const char* str, int size) {
...
FLASHMEM int Read4Bytes(MY_FILE *f)
{
...
FLASHMEM unsigned int StringToWord(const char *string)
{
...
FLASHMEM int towlower2(unsigned int c, Translator *translator)
{
...
FLASHMEM long espeak_rand(long min, long max) {
...
FLASHMEM void espeak_srand(long seed) {
