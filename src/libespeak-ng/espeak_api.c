...
FLASHMEM static espeak_ERROR status_to_espeak_error(espeak_ng_STATUS status)
{
	switch (status)
	{
...
FLASHMEM ESPEAK_API int espeak_Initialize(espeak_AUDIO_OUTPUT output_type, int buf_length, const char *path, int options)
{
	#ifdef ESPEAK_HEAP_HACK2
	initheaphack2();
	#endif
	init_mystdio_FS(); // initialise the virtual file system and add the minimal set of files. 
...
FLASHMEM ESPEAK_API espeak_ERROR espeak_Synth(const void *text, size_t size,
...
FLASHMEM ESPEAK_API espeak_ERROR espeak_Synth_Mark(const void *text, size_t size,
...
FLASHMEM ESPEAK_API espeak_ERROR espeak_Key(const char *key_name)
{
	return status_to_espeak_error(espeak_ng_SpeakKeyName(key_name));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_Char(wchar_t character)
{
	return status_to_espeak_error(espeak_ng_SpeakCharacter(character));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_SetParameter(espeak_PARAMETER parameter, int value, int relative)
{
	return status_to_espeak_error(espeak_ng_SetParameter(parameter, value, relative));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_SetPunctuationList(const wchar_t *punctlist)
{
	return status_to_espeak_error(espeak_ng_SetPunctuationList(punctlist));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_SetVoiceByName(const char *name)
{
	return status_to_espeak_error(espeak_ng_SetVoiceByName(name));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_SetVoiceByFile(const char *filename)
{
	return status_to_espeak_error(espeak_ng_SetVoiceByFile(filename));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_SetVoiceByProperties(espeak_VOICE *voice_selector)
{
	return status_to_espeak_error(espeak_ng_SetVoiceByProperties(voice_selector));
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_Cancel(void)
{
	return status_to_espeak_error(espeak_ng_Cancel());
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_Synchronize(void)
{
	return status_to_espeak_error(espeak_ng_Synchronize());
}

FLASHMEM ESPEAK_API espeak_ERROR espeak_Terminate(void)
	{
	espeak_ERROR r = status_to_espeak_error(espeak_ng_Terminate());
	#ifdef ESPEAK_HEAP_HACK2
	freeheaphack2();
	#endif
	return r;
...
FLASHMEM ESPEAK_API void espeak_CompileDictionary(const char *path, MY_FILE *log, int flags)
{
	espeak_ng_ERROR_CONTEXT context = NULL;
